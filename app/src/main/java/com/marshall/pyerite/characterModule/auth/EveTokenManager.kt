...
class EveTokenManager internal constructor(
    ...
    internal suspend fun <T> executeWithAuthRetry(
        ...
        val firstAuth = authorizationFor(refreshIfNeeded(characterId))
        try {
            return block(firstAuth)
        } catch (httpError: HttpException) {
            if (httpError.code() != 401) throw httpError
        }
        val retryAuth = authorizationFor(forceRefreshInternal(characterId))
    ...
    private fun authorizationFor(result: EveTokenRefreshResult): String =
        when (result) {
            is EveTokenRefreshResult.Unchanged -> result.tokenSet.authorizationHeader()
            is EveTokenRefreshResult.Refreshed -> result.tokenSet.authorizationHeader()
            is EveTokenRefreshResult.NotFound -> error("No token for authenticated ESI call")
            is EveTokenRefreshResult.Failed -> throw result.error
        }
